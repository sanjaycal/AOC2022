...
mnx=min([x[0] for x in elvespos.values()])
mny=min([x[1] for x in elvespos.values()])
mxx=max([x[0] for x in elvespos.values()])
mxy=max([x[1] for x in elvespos.values()])

print(mnx)
print(mny)
print(mxx)
print(mxy)
open("tt.json", "w").write(str(elvespos))

dx = mxx-mnx + 1
dy = mxy-mny + 1

print(dx*dy - len(elvespos.keys()))